...
Entry.find = function find(options) {
  ...
  if (options.after !== undefined &&
    ...
    if (options.sort === 'desc') {
      queryConfig = {
        text: 'SELECT source, ts, data FROM entries WHERE source = $1 ORDER BY ts DESC LIMIT $2 OFFSET $3',
        values: [options.source, options.count, options.startIndex],
        name: 'getChunkDesc'
      };
    } else {
      queryConfig = {
        text: 'SELECT source, ts, data FROM entries WHERE source = $1 ORDER BY ts ASC LIMIT $2 OFFSET $3',
        values: [options.source, options.count, options.startIndex],
        name: 'getChunk'
      };
    }
